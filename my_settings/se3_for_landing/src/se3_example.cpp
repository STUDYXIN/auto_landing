...
#include "se3_for_landing/se3_controller.hpp"
#include "se3_for_landing/se3_dynamic_tuneConfig.h"
...
private:
    ...
    ros::Subscriber odom_sub_, imu_sub_, state_sub_, desire_odom_sub_, desire_angle_sub_, desire_pose_by_ar_sub_;
    ...
    dynamic_reconfigure::Server<se3_for_landing::se3_dynamic_tuneConfig> dynamic_tune_server_;
    dynamic_reconfigure::Server<se3_for_landing::se3_dynamic_tuneConfig>::CallbackType dynamic_tune_cb_type_;
    ...
    void DesirePosebyArCallback(const geometry_msgs::PoseStamped::ConstPtr &msg){
        
        
        if(use_ar_judge == 0)
        {
            desired_state_.p(0) = msg->pose.position.x;
            desired_state_.p(1) = msg->pose.position.y;
            desired_state_.p(2) = msg->pose.position.z + 2.0;
            desired_state_.v.setZero();
            desired_state_.a.setZero();
            desired_state_.j.setZero();

            desired_state_.q.w() = msg->pose.orientation.w;
            desired_state_.q.x() = msg->pose.orientation.x;
            desired_state_.q.y() = msg->pose.orientation.y;
            desired_state_.q.z() = msg->pose.orientation.z;
        ...
        else if(use_ar_judge == 1)
        {
            desired_state_.p(0) = msg->pose.position.x;
            desired_state_.p(1) = msg->pose.position.y;
            desired_state_.p(2) = msg->pose.position.z;
            desired_state_.v.setZero();
            desired_state_.a.setZero();
            desired_state_.j.setZero();
        ...
        //ROS_INFO("desire posit: %f %f %f", desired_state_.p(0), desired_state_.p(1), desired_state_.p(2));
    }

    void DynamicTuneCallback(se3_for_landing::se3_dynamic_tuneConfig &config, uint32_t level){
        ...
        use_ar_judge = config.track_and_landing;
        if (use_ar_judge == -1)
        {
        ...
        // else if(config.track_and_landing == 0)
        // {
        //     geometry_msgs::TransformStamped tfStamped;
        //     try
        //     {
        //     tfStamped = tfBuffer.lookupTransform("map", "tag_0", ros::Time(0));
        //     desired_state_.p(0) = tfStamped.transform.translation.x;
        //     desired_state_.p(1) = tfStamped.transform.translation.y;
        //     desired_state_.p(2) = tfStamped.transform.translation.z;
        //     desired_state_.v.setZero();
        //     desired_state_.a.setZero();
        //     desired_state_.j.setZero();
        //     q.w() = tfStamped.transform.rotation.w;
        //     q.x() = tfStamped.transform.rotation.x;
        //     q.y() = tfStamped.transform.rotation.y;
        //     q.z() = tfStamped.transform.rotation.z;
        //     ROS_INFO("Apriltag posit: %f %f %f", desired_state_.p(0), desired_state_.p(0), desired_state_.p(0));
        //     }
...
public:
    ...
    void init(ros::NodeHandle &nh){
        ...
        desire_pose_by_ar_sub_ = nh.subscribe<geometry_msgs::PoseStamped>("/tag_0_pose", 10, &SE3_EXAMPLE::DesirePosebyArCallback, this);
        ...
        // hover_percent_ = 0.25;
        // max_hover_percent_ = 0.35;

        hover_percent_ = 0.4;
        max_hover_percent_ = 0.8;

        desired_state_.p(0) = -3.0;
        desired_state_.p(1) = 0.0;
        desired_state_.p(2) = 1.0;
